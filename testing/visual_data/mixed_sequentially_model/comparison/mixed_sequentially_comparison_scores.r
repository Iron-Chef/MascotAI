...
line_graph <- ggplot(data = model_performance, aes(x = Iteration, y = PerformanceMetric, group = Dataset, color = Dataset)) +
  ...
  labs(title = "Mixed Sequentially Model Comparison Scores",
       x = "Iteration",
       y = "Number of Correct Comparisons",
       color = "Dataset") + 
...
# Save the line graph
ggsave("mixed_sequentially_model_comparisons.png", plot = line_graph, width = 12, height = 6, dpi = 600)
